<template>
  <div>
    <div class="page-header d-flex justify-content-between p-4 border my-2">
      <h3 class="col-12 col-md-4">Egresos</h3>
      <ul class="list-group col-12 col-md-6">
        <li class="list-group-item">
          <h5 class="text-dark font-weight-bold">Total egresos: {{ expensesTotal.price | currency }}</h5>
        </li>
      </ul>
      <!-- <button type="button" class="btn btn-primary col-12 col-md-4" data-toggle="modal" data-target="#expenseModal"
        v-if="$root.validatePermission('expense-store')"> -->
      <div class="col-12 col-md-2">
        <button type="button" class="btn btn-primary btn-block" data-toggle="modal" data-target="#expenseModal">
          Crear Egreso
        </button>
      </div>
    </div>
        ...
        <table class="table table-bordered table-sm">
          ...
          <tbody v-if="expenseList.data && expenseList.data.length > 0">
            <tr v-for="e in expenseList.data" :key="e.id">
              <td>{{ e.user.name }} - {{ e.user.username }}</td>
              <td>{{ e.date }}</td>
              <td>{{ e.type_output }}</td>
              <td class="wrap">
                <textarea name="" class="form-control-plaintext" readonly id="" cols="7" rows="2" v-model="e.description">
                </textarea>
              </td>
          ...
          <div v-else class="w-100"> No se han registrado datos</div>
        </table>
