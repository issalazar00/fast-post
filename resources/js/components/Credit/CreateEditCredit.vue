      ...
      <section>
        <div>
          ...
          >
            ...
            <tbody v-if="productsCreditList.length > 0">
              <tr v-for="(p, index) in productsCreditList" :key="p.id">
                <td>
                  <button
...
export default {
  ...
  computed: {
    total_tax_exc: function () {
      var total = 0.0;
      this.productsCreditList.forEach(
        (product) =>
    ...
    total_discount: function () {
      var total = 0.0;
      this.productsCreditList.forEach((product) => {
    ...
    payment_return: function () {
      ...
      if (this.credit.cash > 0) {
        if(this.credit.pay_payment > this.credit.total_tax_inc){
          ...
          alert("No puedes abonar una cantidad superior al valor de la factura");
  ...
  methods: {
    listItemsCredit() {
      ...
      axios
        .get(`api/credits/${me.credit_id}`, this.$root.config)
        .then(function (response) {
          me.credit.id_client = response.data.credit_information.client_id;
          me.credit.client = response.data.credit_information.client.name;

          me.productsCreditList = response.data.credit_details;
        });
    },
    searchProduct() {
      let me = this;
    ...
    addProduct(new_product) {
      ...
      me.productsCreditList.filter((prod) => {
    ...
    removeProduct(index, detail_id = null) {
      this.productsCreditList.splice(index, 1);
      if (detail_id != null || detail_id != 0) {
        axios.delete(`api/credit-details/${detail_id}`, this.$root.config);
      }
    },
    searchClient() {
      let me = this;
    ...
    createOrUpdateCredit(state_credit) {
      ...
      if (this.productsCreditList.length > 0) {
        this.credit.productsCredit = this.productsCreditList;
        if (this.credit_id != 0 && this.credit_id != null) {
          axios
            .put(
              `api/credits/${this.credit_id}`,
              this.credit,
              this.$root.config
            )
          ...
          axios
            .post(`api/credits`, this.credit, this.$root.config)
    ...
    commands() {
      ...
      shortcut.add("F1", function () {
        me.createOrUpdateCredit(2);
      });

      shortcut.add("F2", function () {
        me.createOrUpdateCredit(4);
      });
